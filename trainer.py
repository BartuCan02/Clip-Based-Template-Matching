...
class Matching_Trainer(LightningModule):
    ...
    def each_step_multi_exemplars(self, batch, stage):
        ...
        Replaces self.each_step in __init__ when --num_exemplars > 1 is combined
        with --eval. Each exemplar goes through the model separately and the
        predictions are concatenated before refinement and NMS, so the exemplars
        compete in one NMS pass instead of giving independent detection sets.
        Only works with batch_size 1.

        Note this never exercises the text path. It predates the 1025-channel work
        and calls the model without a naclip_heatmap, so the extra channel always
        falls back to the learned no_text_token. Use each_step for anything
        text-conditioned.
        """
        image = batch["image"]
    ...
    def each_step(self, batch, stage):
        """
        Main train/val/test step, the 1025-channel path.

        Picks the query mode, builds the NaCLIP heatmap for it, runs TMR and the
        losses, and on val/test decodes boxes and logs them.

        Query mode:
        - with --use_modality_dropout, training draws uniformly over
          box_and_text / box_only / text_only per sample, which is what lets one
          checkpoint serve all three modes at eval
        - otherwise the mode is fixed by --input_mode
        - use_box / use_text go to the model, which swaps in its learned
          no_box_token / no_text_token for the missing modality, so the head
          always sees a full 1025-channel tensor

        Text conditioning:
        - with --use_naclip_heatmap and text in play, batch["label"] becomes a
          dense [B, 1, H, W] map via the two-class softmax against "background"
        - built per sample in a loop, so it scales with batch size and dominates
          step time

        Watch the asymmetry: use_text=False skips NaCLIP, but so does
        --use_naclip_heatmap=False even in a text mode, and then text_only is
        left with nothing but the placeholder.
        """
        image = batch["image"]
