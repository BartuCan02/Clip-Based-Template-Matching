...
class NACLIPHeatmap:
    def __init__(self,clip_path="ViT-B/16",device="cuda",arch="reduced",attn_strategy="naclip",gaussian_std=5.0,logit_scale=40,):
        ...
        Turns a class name into the dense [B, 1, H, W] probability map used as the
        1025th channel. Inference only, the CLIP weights stay frozen.

        clip_path:
        - ViT-B/16 gives a 14x14 patch grid at 224px, resized to the TMR features

        arch / attn_strategy:
        - 'reduced' drops the FFN and residual in the last block, which is what
          makes the patch tokens locally meaningful instead of globally pooled
        - 'naclip' applies the Gaussian neighbourhood attention bias

        gaussian_std:
        - std of that neighbourhood prior, in patch units, must be > 0
        - 5.0 is NaCLIP's default and the value used in the report, without it the
          map is dominated by high-frequency noise and outlier patches

        logit_scale:
        - temperature of the two-class softmax against "background", higher
          sharpens towards 0/1
        - 40 for all reported results, never ablated, so treat it as a knob
        """
        self.device = device
    ...
    def target_heatmap(self, img, class_names, target_idx=1, out_size=None):
        ...
        naclip_heat_map = probs[:, target_idx:target_idx + 1]

        if out_size is not None:    #upsampling
            naclip_heat_map = F.interpolate(
                naclip_heat_map,
                size=out_size,
                mode="bilinear",
                align_corners=False,
